...
always@(posedge sys_clk or negedge sys_rst_n) begin
    if( sys_rst_n == 1'b0)
        lcd_init_done <= 1'b0;
    else if( tft_screen_init_ack == 1'b1)   //初始化模块响应，lcd_init_done置位
        lcd_init_done <= 1'b1;
    else
...
spi_tft_screen_flush #(
    ...
    .spi_screen_flush_updte_o           (spi_screen_flush_updte_o  ),//屏幕数据更新
    ...
    .spi_send_flush_req_o               (spi_send_flush_req        ),//spi发送请求
    .spi_send_flush_end_o               (spi_send_flush_end        ),//结束spi发送
    .spi_send_flush_ack_i               (spi_send_flush_ack        ) //spi一次数据发送完成
);
...
spi_tft_screen_init  #(
    ...
    .tft_screen_init_ack_o              (tft_screen_init_ack       ),//初始化响应
    .tft_screen_init_data_o             (tft_screen_init_data      ),//初始化数据
    .tft_screen_init_dc_o               (tft_screen_init_dc        ),//初始化dc


    .spi_send_init_req_o                (spi_send_init_req         ),//spi发送请求
    .spi_send_init_end_o                (spi_send_init_end         ),//结束spi发送
    .spi_send_init_ack_i                (spi_send_init_ack         ) //spi一次数据发送完成
);
...
spi_master_driver spi_master_driver_inst(
    ...
    .spi_send_ack_o                     (spi_send_ack              ),// spi发送8bit完成信号

    .lcd_dc_i                           (lcd_dc_i                  ),//数据/命令信号输入
    .lcd_dc                             (lcd_dc                    ),//数据/命令信号输出
